...
#define TEST_APP_SPI_ID_MASTER APP_SPI_ID_MASTER
#else
#define TEST_APP_SPI_ID_MASTER APP_SPIM_ID
#endif
...
static void spi_flash_write_enable(void)
{
    ...
    app_spi_transmit_sync(TEST_APP_SPI_ID_MASTER, &cmd, 1, 1000);
}
...
static uint8_t spi_flash_read_status(void)
{
    ...
    app_spi_read_eeprom_sync(TEST_APP_SPI_ID_MASTER, &cmd, &state_reg, 1,  1, 1000);
...
uint32_t spi_flash_read_id(void)
{
    ...
    app_spi_read_eeprom_sync(TEST_APP_SPI_ID_MASTER, &cmd, &data[0], 1,  3, 1000);
...
void spi_flash_sector_erase(uint32_t dst)
{
    ...
    app_spi_transmit_sync(TEST_APP_SPI_ID_MASTER, &ctl_frame[0], 4, 1000);
...
void spi_flash_page_program(uint32_t dst, uint8_t *data, uint32_t nbytes)
{
    ...
    spi_flash_wait_busy();
    g_master_tdone = 0;
    app_spim_dma_transmit_with_ia(TEST_APP_SPI_ID_MASTER, SPI_FLASH_CMD_PP, dst, data, nbytes);
...
{
    g_master_trdone = 0;
    app_spim_dma_receive_with_ia(TEST_APP_SPI_ID_MASTER, SPI_FLASH_CMD_READ, dst, 0, buffer, nbytes);
...
{
    g_master_trdone = 0;
    app_spim_dma_receive_with_ia(TEST_APP_SPI_ID_MASTER, SPI_FLASH_CMD_FREAD, dst, 1, buffer, nbytes);
    while(g_master_trdone == 0);
}
